...
def train_classifier(tokenizer1, encoder1, data, hparams, tokenizer2=None, encoder2=None):

    train_dataset, val_dataset, test_dataset = prepare_datasets(data)
    batch_collater = partial(tweet_batch_collate, tokenizer1=tokenizer1, encoder1=encoder1, tokenizer2=tokenizer2, encoder2=encoder2)
...
def get_hparams(tune):
    ...
    if tune == True:
        ...
        hparam_sets = [{'hidden_dim': 256, 'learning_rate': 5e-3, 'num_epochs': 20, 'batch_size': 32, 'n_layers': 0}]
...
if __name__ == '__main__':
    ...
    hparam_sets = get_hparams(args.tune)
    for hparams in hparam_sets:
        if args.model == 'simcse' or args.model == 'bert':
            ...
            encoder = AutoModel.from_pretrained(models[args.model])
            hparams['input_dim'] = encoder.embeddings.token_type_embeddings.embedding_dim
            train_classifier(tokenizer, encoder, twitter_data, hparams)

        elif args.model == 'both':
            tokenizer1 = AutoTokenizer.from_pretrained(models['simcse'])
            encoder1 = AutoModel.from_pretrained(models['simcse'])
            tokenizer2 = AutoTokenizer.from_pretrained(models['bert'])
            encoder2 = AutoModel.from_pretrained(models['bert'])
            ...
            train_classifier(tokenizer1, encoder1, twitter_data, hparams, tokenizer2, encoder2)
